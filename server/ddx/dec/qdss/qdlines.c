    ...
    register DDXPointPtr pptInit;
{
    ...
    BoxPtr		pclip;		/* used for trivial reject */
    int			nclip;		/* used for trivial reject */
    RegionPtr		pdrawreg = ((QDPrivGCPtr)pGC->devPriv)->pCompositeClip;
    ...
    nclip = 0;
    pclip = (BoxPtr) alloca( pdrawreg->numRects * sizeof(BoxRec));
    for ( ir=0; ir<pdrawreg->numRects; ir++)
	if ( ! NOINTERSECTION( &ptbounds, &pdrawreg->rects[ir]))
	    pclip[ nclip++] = pdrawreg->rects[ir];
