    ...
    int on;
{
...
#ifdef QIOVIDEOON
	(void) ioctl(fdQVSS, QIOVIDEOON, 0);
#endif
	return TRUE;
    }
    else
    {
#ifdef QIOVIDEOOFF
	(void) ioctl(fdQVSS, QIOVIDEOOFF, 0);
	return TRUE;
#else
        return FALSE;
#endif
...
#ifdef ULTRIX3_0
    if (isSMdevice)
	retval = mfbScreenInit(index, pScreen, smInfo->bitmap, 
			       1024, 864, 80, 80);
    else
#endif
    ...
    CursorPtr	pCurs;
{
    ...
    if (isSMdevice)
    {
	if ( ioctl (fdQVSS, QIOWCURSOR, ((short *)pCurs->devPriv[ pScr->myNum]))
	    < 0)
	{
