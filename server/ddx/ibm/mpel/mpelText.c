...
/* $Header: /andrew/X11/R3src/tape/server/ddx/ibm/mpel/RCS/mpelText.c,v 6.3 88/10/25 01:58:36 kbg Exp $ */
/* $Source: /andrew/X11/R3src/tape/server/ddx/ibm/mpel/RCS/mpelText.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /andrew/X11/R3src/tape/server/ddx/ibm/mpel/RCS/mpelText.c,v 6.3 88/10/25 01:58:36 kbg Exp $";
#endif
    ...
    FontEncoding	 fontEncoding;
{
    ...
    pRegion = ( (ppcPrivGC *) (pGC->devPriv) )->pCompositeClip;
    ...
    FontEncoding	 fontEncoding;
{
    ...
    x += (xOffset=((WindowPtr)pDraw)->absCorner.x);
    y += ((WindowPtr)pDraw)->absCorner.y;
...
char		*chars;
{
	...
	return(mpelPolyText( pDraw, pGC, x, y, count, chars,  Linear16Bit ));
    else
	return(mpelPolyText( pDraw, pGC, x, y, count, chars,  TwoD16Bit ));
...
char		*chars;
{
    TRACE(("mpelImageText16( 0x%x, 0x%x, (%d,%d), %d, 0x%x )\n",
						pDraw,pGC,x,y,count,chars));
    if (pGC->font->pFI->lastRow==0)
	mpelImageText( pDraw, pGC, x, y, count, chars,  Linear16Bit );
    else
	mpelImageText( pDraw, pGC, x, y, count, chars,  TwoD16Bit );
    return;
}
