...
******************************************************************/
/* $Header: vgaImages.c,v 6.0 88/08/18 08:57:09 erik Exp $ */
/* $Source: /vice/X11/r3/server/ddx/ibm/vga/RCS/vgaImages.c,v $ */

#ifndef lint
static char *rcsid = "$Header: vgaImages.c,v 6.0 88/08/18 08:57:09 erik Exp $";
#endif
...
void
vgaDrawColorImage( x, y, w, h, data, alu, planes )
int x, y ;
register int w, h ;
...
/* Round up to multiple of four (4) pixels. i.e. 32-bits */
RowIncrement = ( w + 3 ) & ~3 ;
...
for ( DataPtr = data ;
	...
	for ( dst = StartByte, src = DataPtr,
	      ...
	      Pixel_Count-- ;
	      src++ ) {
		SetVideoGraphicsData( currMask ) ; /* Set The Bit Mask Reg */
...
void
vgaReadColorImage( x, y, lx, ly, data )
int x, y ;
int lx, ly ;
...
pad = ( skip - ignore ) & 0x3 ;
src = (volatile unsigned char *) SCREENADDRESS( x, y ) ;
...
if ( center_width < 0 )
	...
	for ( savCenterWidth = center_width ;
	      ly-- ;
	      center_width = savCenterWidth, src += BYTES_PER_ROW ) {
		tmp = read8Z( src++ ) ;
		if ( dx = skip ) {
			tmp >>= ( dx << 2 ) ;
			center_width++ ;
